...
module Blogger
  ...
  class Renderer < Redcarpet::Render::HTML
    ...
    def block_code(code, language)
      if language == 'mermaid'
        %(<pre class="mermaid">#{Haml::Util.escape_html(code)}</pre>)
      else
        super
      end
    end
    ...
    def _build_contents(src = nil, dst = nil)
      ...
      Dir[File.join(src, '*')].each do |f|
        if File.directory?(f)
          ...
          context.articles = @nav.except('about_me').sort.reverse
          context.articles.each do |ar|
            ar.each do |pages|
              if pages.is_a?(Array)
                pages.sort! { |a, b| b[:date] <=> a[:date] }
                pages.reject! { |a| a[:title] == 'About Me' }
              end
            end
          end
